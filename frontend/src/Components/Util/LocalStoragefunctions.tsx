// Uma observação que eu posso fazer e que você talvez tenha percebido é que eu defino o tipo duas fezes uma usando LocalStorageP e a outra é quando eu defino o tipo das promps que a função recebe, mas é possível fazer esse processo uma única vez, se eu remover as definições na função e usar o type LocalStorageP para definir os tipos dos promps só preciso definir o type é pronto certo? Errado, se eu fizer isso eu teria que definir o tipo das promps no componente pai que estar mandando as promps para o componente filho(se já não foram definidas lá), então dessa maneira eu diminuo o tamanho do código que seria escrito no componente pai por ter que definir o tipo das promps enquanto as envia para cá.
// LocalStorageP 
type LocalStorageP = {
    chave:string;
    id:number;
